...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="UFM: Unified Dense Correspondence with Flow",
        prog="ufm"
    )
...
def launch_demo(args):
    ...
    try:
        ...
        from gradio_demo import initialize_model, create_demo
...
def run_inference(args):
    ...
    try:
        ...
        cv2.imwrite(str(output_dir / "flow_visualization.png"), 
                   cv2.cvtColor(flow_vis_img, cv2.COLOR_RGB2BGR))
        
        # Save covisibility mask
        cv2.imwrite(str(output_dir / "covisibility_mask.png"), 
                   (covisibility * 255).astype(np.uint8))
        
        # Save warped image
        warped = warp_image_with_flow(source_rgb, flow)
        cv2.imwrite(str(output_dir / "warped_source.png"),
